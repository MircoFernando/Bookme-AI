...
## Corpus Check
- 40 files · ~19,141 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 419 nodes · 758 edges · 19 communities (16 shown, 3 thin omitted)
...
Cohesion: 0.29
Nodes (6): LogRecord, _InterceptHandler, Centralised logging — powered by **loguru**.  Usage (any module)::      from log, Route stdlib ``logging`` records into loguru so third-party libs     (uvicorn, h, Configure loguru for the current process. Call once at each entry-point.      Ar, setup_logging()

### Community 6 - "Decision graph & routing — architecture notes"
Cohesion: 0.04
Nodes (43): 10. Phase 4 status & acceptance, 11. Repository map (Phase 4 agents), 12. Questions for Week 13 / instructor (optional prompt), 1. Two-stage pipeline (mental model), 2. Week 13 architecture choice (two state types), 3. Decision graph topology (TripWeaver), 4.1 Both branches always start, 4.2 Fan-in before `decide` (+35 more)

### Community 7 - "TripWeaver — Development Roadmap & Phase Log"
Cohesion: 0.10
Nodes (23): AgentOrchestrator, AgentResponse, _async_mcp_dispatch(), _format_session_memory(), _last_user_text(), _mcp_result_to_str(), _MCPFlightToolAdapter, _MCPHotelToolAdapter (+15 more)
...
Cohesion: 0.07
Nodes (35): ChatOpenAI, build_agent_mcp(), build_orchestrator(), In-process ``HotelTool`` / ``FlightTool`` (debug without MCP subprocesses)., Assessment path — agents call Convex only through MCP stdio servers., Infrastructure layer — pure plumbing (config, logging, LLM clients, observabilit, _build_llm(), get_chat_llm() (+27 more)
...
- **Why does `build_orchestrator()` connect `infrastructure/llm.py` to `http_client.py`, `flight_server.py`, `TripWeaver — Development Roadmap & Phase Log`?**
  _High betweenness centrality (0.094) - this node is a cross-community bridge._
- **Why does `SessionStore` connect `infrastructure/llm.py` to `TripWeaver — Development Roadmap & Phase Log`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `AgentOrchestrator` connect `TripWeaver — Development Roadmap & Phase Log` to `router.py`, `infrastructure/llm.py`?**
